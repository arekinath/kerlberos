...
-export([des_string_to_key/2, des_fix_parity/1]).
-export([crc/1, crc/2]).
...
-record(etype, {
	mac_fun :: {atom(), atom(), [term()]},
	mac_length :: integer(),
	block_cipher :: atom(),
	block_length :: integer(),
	key_length :: integer(),
	iv :: atom() | term(),
	string_to_key :: {atom(), atom(), [term()]},
	random_to_key :: {atom(), atom(), [term()]}
}).
etype_params(des_crc) -> #etype{
	mac_fun = {?MODULE, crc, []}, mac_length = 4,
	block_cipher = des_cbc, block_length = 8, key_length = 8,
	iv = key,
	string_to_key = {?MODULE, des_string_to_key, []},
	random_to_key = {?MODULE, des_fix_parity, []}
	};
etype_params(des_md4) -> #etype{
	mac_fun = {crypto, hash, [md4]}, mac_length = 8,
	block_cipher = des_cbc, block_length = 8, key_length = 8,
	iv = <<0:64>>,
	string_to_key = {?MODULE, des_string_to_key, []},
	random_to_key = {?MODULE, des_fix_parity, []}
	};
etype_params(des_md5) -> #etype{
	mac_fun = {crypto, hash, [md5]}, mac_length = 8,
	block_cipher = des_cbc, block_length = 8, key_length = 8,
	iv = <<0:64>>,
	string_to_key = {?MODULE, des_string_to_key, []},
	random_to_key = {?MODULE, des_fix_parity, []}
	};
etype_params(_) -> error(unknown_etype).

encrypt(EType, Key, Data) ->
	E = etype_params(EType),
	Confounder = crypto:rand_bytes(E#etype.block_length),
	PreMAC = pad_block(<<Confounder/binary, 0:(E#etype.mac_length)/unit:8, Data/binary>>,
		E#etype.block_length),
	{MacM, MacF, MacA} = E#etype.mac_fun,
	MAC = erlang:apply(MacM, MacF, MacA ++ [PreMAC]),
	IV = case E#etype.iv of
		key -> Key;
		B when is_binary(B) -> B
	end,
	PostMAC = pad_block(<<Confounder/binary, MAC/binary, Data/binary>>,
		E#etype.block_length),
	crypto:block_encrypt(E#etype.block_cipher, Key, IV, PostMAC).
...
string_to_key(EType, String, Salt) ->
	#etype{string_to_key = F} = etype_params(EType),
	{SF, SM, SA} = F,
	erlang:apply(SF, SM, SA ++ [String, Salt]).

random_to_key(EType, Data) ->
	#etype{random_to_key = F} = etype_params(EType),
	case F of
		identity -> Data;
		{SF, SM, SA} ->
			erlang:apply(SF, SM, SA ++ [Data])
	end.
